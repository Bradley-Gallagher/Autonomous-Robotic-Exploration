...
#Changes for Realworld Operations
#Ln. 192-194 - Adjust Occupancy grid size due to reducing resolution
#Ln. 218-219 - Adjust Centre of Occupancy Grid
#Ln. 229-230 - Adjust Centre of Occupancy Grid
#Ln. 356 - Reduce default laser update distance when no wall detected (smaller max due to smaller world)

#Ln. 565-570 - Adjust Mask Boundaries (STILL NEEDS TESTING & ADJUSTMENTS)

#Ln. 618, 622, 640, 644 - Change Turn Multiplier for Mask Centroid offset/error

#Ln. 655-665 - (NEEDS TO BE REDONE WITH IN LAB TESTING FOR BOUNDARIES)
#Ln. 680 - (NEEDS TO BE REDONE WITH IN LAB TESTING FOR BOUNDARIES)
#Ln. 689-705 - (NEEDS TO BE REDONE WITH IN LAB TESTING FOR BOUNDARIES)
#Ln. 727-742 - Adjusted the max value for Occupancy Grid
#Ln. 785 - Reduced acceptable tolerance for MoveBase
#Ln. 1017 - Reduced distance required for visual exploration to 1 due to smaller size of map
    ...
    def iterateoccupy(self, las, lasangle):
        ...
        if isinstance(las, float):
            ...
            if str(las) == 'inf' or las == 0.0:
                lasdist = 1 #Downgrade to 1 if unsure
...
class camdata:
    def __init__(self):
        self.imagesub = rospy.Subscriber('/camera/color/image_raw', Image, self.camera_callback) #/color for bot /rgb for sim
    ...
    def gotored(self, redobjno):
        ...
        if laser.front > 0.7 or laser.front == 0:
            mover.linear.x = 0.2
            mover.angular.z = -cam.rederrorx / 10
            pub.publish(mover)
    ...
    def bluecheck(self):
        if self.blueerrorx != -20 and self.blueerrory != 20 and self.blueerrory > 30 and self.blueerrorx > -55 and self.blueerrorx < 55:
            return True
        else:
            return False
...
class MoveBaseActionList:
    ...
    def tolerance(self, goalx, goaly):
        if odom.prevx < goalx+0.3 and odom.prevx > goalx-0.3 and odom.prevy < goaly+0.3 and odom.prevy > goaly-0.3:
            return True
        else:
            return False
